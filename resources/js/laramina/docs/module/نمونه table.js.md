...
> **نکته:** اگر جدول هیچ ستون فیلترپذیری (مثل `is_active` یا `is_default`) نداشته باشد، ماژول تولیدشده با دستور `laramina:make-ui` به‌جای بخش خالی، یک نمونهٔ **غیرفعال** (کامنت‌شده) از فیلتر `status` می‌سازد. برای فعال‌سازی کافی است کامنت‌ها را بردارید و `key` را با ستون واقعی جدول جایگزین کنید.

```js
    filters: [
        // نمونهٔ غیرفعال (در ماژول‌های بدون ستون فیلترپذیر):
        // {
        //     key: 'status',
...
نمونهٔ فیلترهای واقعی (وقتی ستون فیلترپذیر وجود داشته باشد):
